...
export function createStackTrace(error: Error): Promise<Array<Sleuren.StackFrame>> {
    return new Promise(resolve => {
        if (!hasStack(error)) {
            assert(false, "Couldn't generate stacktrace of below error:", sleuren.debug);
        ...
        Promise.all(
            ErrorStackParser.parse(error).map(frame => {
                return new Promise<Sleuren.StackFrame>(resolve => {
